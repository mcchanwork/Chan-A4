...
function stats(req, res)
{
  ...
  con.connect(function(err) 
  {
    ...
    {
      con.query("SELECT MIN(GAME_GUESSES) AS best, MAX(GAME_GUESSES) AS worst, COUNT(GAME_ID) AS gamesPlayed FROM GAME ORDER BY GAME_GUESSES", function (err, result, fields) 
      {
        if (err) 
          writeResult(req, res, {'error' : err});
        else
